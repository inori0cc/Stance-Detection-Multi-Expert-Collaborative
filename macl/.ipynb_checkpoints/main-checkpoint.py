...
from utils.Losses import simCSELoss, vaeLoss, SupConLoss, contrastive_loss
...
class Instructor:
    def __init__(self, opt) -> None:
        ...
        self.max_f1_1_val = 0
        self.max_f1_2_val = 0
        self.best_val_metrics = None#记录最佳验证集指标
        self.best_test_metrics = None#记录最佳测试集指标
        ...
        self.vae_criterion = vaeLoss(opt)
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for _, sample_batched in enumerate(self.train_data_loader):
                ...
                targets, texts, stances, input_idss, attention_masks, token_type_idss, cl_input_idss, cl_attention_masks, cl_token_type_idss, \
                    = sample_batched
                if len(stances) == 1:  # self.opt.batch_size:
                    continue
                ...
                predict, feats, feats_hat, mu, log_var, moe_loss = self.model(inputs)
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for _, v_sample_batched in enumerate(self.val_data_loader):
                ...
                predict, z, dz, mu, logvar, moe_loss = self.model(inputs)
